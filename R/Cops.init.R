#function to create and auto fill init.data and info.file
...
Cops.init.R <- function(dirdats){
	...
	for(dirdat in dirdats) {
		...
		cast <- fread(file.path(dirdat, list.files(dirdat, pattern = "CAST")[1]), skip = "End of")
		...
		init.params[18] <- str_replace(init.params[18], "(?<=[:graph:]{0,100};[:graph:]{0,100};).+", str_c(time.interval.for.smoothing.optics, collapse = ","))
