...
l2_water_sample_gen <- function(project, Var = c("Ag","Ap")) {
	...
	CheckList <- check_project(project, L1, L2, set="Lab")
	...
	if (any(str_detect(Var, "Ag"))){
		...
		AgFiles <- list.files(path = file.path(L1,"Ag","RData"), "[[:digit:]]{3}([[:alpha:]])?.RData",
						  full.names = T)
		...
		if (any(names(AgLog) == "Boat")) {
			AgLog <- AgLog %>%
				mutate(L2path = file.path(L2,
									 paste0(gsub("-", "", as.character(lubridate::date(Date))),
									 	  "_Station",Station),
	...
	if (any(str_detect(Var, "Ap"))) {
		# Match sample ID, station name along with L1path
		ApFiles <- list.files(path = file.path(L1,"Ap_nap","RData"), "[[:digit:]]{3}([[:alpha:]])?.RData",
						  full.names = T)
